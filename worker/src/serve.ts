...
<form method="post" action="/${address}/__unlock"><b>&#128274; ${address}</b><div class="field"><input type="password" name="password" aria-label="password" placeholder="password" maxlength="256" autofocus autocomplete="current-password"><button>unlock</button></div>${
...
async function handleUnlock(
  ...
  if (contentLength === null) {
    return htmlResponse(unlockForm(address, "request size required"), 411);
  }
  const declaredBytes = Number(contentLength);
  if (!Number.isSafeInteger(declaredBytes) || declaredBytes < 0) {
    return htmlResponse(unlockForm(address, "invalid request size"), 400);
  }
  if (declaredBytes > MAX_UNLOCK_BODY_BYTES) {
    return htmlResponse(unlockForm(address, "request too large"), 413);
  }
  ...
  if (bytes.length !== declaredBytes) {
    return htmlResponse(unlockForm(address, "request size mismatch"), 400);
  }
  const contentType = req.headers.get("content-type")?.split(";", 1)[0].trim()
    .toLowerCase();
  if (contentType !== "application/x-www-form-urlencoded") {
    return htmlResponse(unlockForm(address, "unsupported form"), 415);
  }
  const password = new URLSearchParams(new TextDecoder().decode(bytes)).get(
    "password",
  );
  if (password !== null && password.length > MAX_PASSWORD_LENGTH) {
    return htmlResponse(unlockForm(address, "password too long"), 400);
  }
  if (
